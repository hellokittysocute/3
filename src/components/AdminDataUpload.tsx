...
const HEADER_MAP: Record<string, { field: string; type: 'string' | 'number' }> = {
  ...
  '제조요청여부': { field: 'production_request_yn', type: 'string' },
  '제조요청': { field: 'production_request_yn', type: 'string' },
  '제조 1차': { field: 'mfg1', type: 'string' },
  '제조1차': { field: 'mfg1', type: 'string' },
  '기존제조': { field: 'mfg1', type: 'string' },
  '현재제조': { field: 'mfg1', type: 'string' },
  '현재 제조계획': { field: 'mfg1', type: 'string' },
  '현재제조계획': { field: 'mfg1', type: 'string' },
...
function parseCSVToRows(csvText: string): ParseResult {
  ...
  const rows = dataLines.map((line, index) => {
    ...
    return {
      ...
      production_request_yn: getVal(cols, headerMapping, '제조요청여부', '제조요청'),
      mfg1: getVal(cols, headerMapping, '제조 1차', '제조1차', '기존제조', '기존 제조', '현재제조', '현재 제조계획', '현재제조계획'),
